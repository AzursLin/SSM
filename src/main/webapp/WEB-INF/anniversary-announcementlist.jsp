...
<div class="container">
    <div class="row clearfix">
        ...
        <div class="anniversary-content pull-right index-transparent">
            <p class="content-title">校庆公告：</p>
            <c:if test="${!empty AnniversaryAnnouncementList}">
                <div class="dowebok">
                    <ul>
                        <c:forEach items="${AnniversaryAnnouncementList}" var="AnniversaryAnnouncement">
                            <a href="anniversary-announcement?id=${AnniversaryAnnouncement.id}&code=${AnniversaryAnnouncement.creatTime}"><li class="specific-content-list"> ${AnniversaryAnnouncement.title}</li></a>
                        </c:forEach>
                    </ul>
                </div>
            </c:if>
            <p> 当前页数${PageInfo.pageNum} </p>
            <c:if test="${PageInfo.pageNum > 1}">
            <a href="anniversary-announcementlist?page=${PageInfo.pageNum-1}">  上一页  </a>
            </c:if>
            <c:if test="${PageInfo.nextPage > 1}">
            <a href="anniversary-announcementlist?page=${PageInfo.nextPage}">  下一页  </a>
            </c:if>
            <p>总计${PageInfo.total} </p>
        </div>
    </div>
    <div class="row clearfix">
